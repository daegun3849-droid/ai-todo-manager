...
export default function Home() {
  ...
  return (
    <main className="flex min-h-screen flex-col items-center p-5 bg-[#F8FAFC]">
      <h1 className="text-3xl font-black mb-6 text-slate-800 tracking-tight">✨ AI로 내 계획 일정 서비스</h1>

      {/* 👤 환영 박스 */}
      <div className="w-full max-w-md mb-6">
        {user && (
          <div className="flex flex-col items-center p-6 bg-white rounded-[2rem] shadow-sm border border-slate-100 text-center">
            <p className="text-lg font-black text-slate-700">반갑습니다, <span className="text-blue-600">{user.email?.split('@')[0]}</span>님! 👋</p>
      ...
      <div className="w-full max-w-md mb-6 bg-white p-5 rounded-[2rem] shadow-sm border border-slate-100">
        <div className="flex justify-between items-end mb-2 px-1">
          <span className="text-sm font-black text-slate-600">오늘의 달성률</span>
      ...
      <div className="w-full max-w-md bg-white p-6 rounded-[2.5rem] shadow-xl border border-slate-50 mb-10 space-y-4">
        <input type="text" value={inputValue} onChange={(e) => setInputValue(e.target.value)} placeholder="오늘 해치울 일은?" className="w-full p-4 bg-slate-50 rounded-2xl font-bold text-lg outline-none" />
        <textarea value={descriptionValue} onChange={(e) => setDescriptionValue(e.target.value)} placeholder="상세 플랜을 적어보세요" className="w-full p-4 bg-slate-50 rounded-2xl text-sm h-20 resize-none outline-none" />
        
        <div className="space-y-3">
          <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
            <label className="text-[11px] font-black text-green-600 block mb-1">🟢 시작 일시</label>
            <input type="datetime-local" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="w-full bg-transparent font-bold text-sm outline-none" />
          </div>
          <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
            <label className="text-[11px] font-black text-red-500 block mb-1">🚨 마감 일시</label>
            <input type="datetime-local" value={dueDate} onChange={(e) => setDueDate(e.target.value)} className="w-full bg-transparent font-bold text-sm outline-none" />
          </div>
        </div>
          ...
          <div key={todo.id} className={`p-6 rounded-[2.5rem] bg-white shadow-lg border border-slate-50 flex flex-col gap-4 relative transition-all ${todo.completed ? 'opacity-50' : ''}`}>
            ...
            <div className="flex items-start gap-3 pr-10">
              ...
              <div className="flex-1 overflow-hidden">
                <h3 className={`font-black text-xl leading-tight break-all ${todo.completed ? 'line-through text-slate-300' : 'text-slate-800'}`}>{todo.title}</h3>
                {todo.description && (
                  <div className="text-[13px] text-blue-600 bg-blue-50/50 p-4 rounded-2xl mt-3 font-bold border border-blue-100/30 break-all">
                    {todo.description}
                  </div>
                )}
              </div>
            ...
            {/* 📅 [해결됨] 깨지지 않는 시작/마감 레이아웃 */}
            <div className="flex flex-col gap-2 pt-4 border-t border-slate-50">
              <div className="flex items-center gap-3 px-3 py-2 bg-green-50/50 rounded-xl whitespace-nowrap overflow-hidden">
                <span className="text-[11px] font-black text-green-600 flex-shrink-0">🟢 시작</span>
                <span className="text-[13px] font-bold text-slate-700 truncate">
                  {new Date(todo.created_date).toLocaleString('ko-KR', {month:'short', day:'numeric', hour:'2-digit', minute:'2-digit'})}
                </span>
              </div>
              {todo.due_date && (
                <div className="flex items-center gap-3 px-3 py-2 bg-red-50/50 rounded-xl whitespace-nowrap overflow-hidden">
                  <span className="text-[11px] font-black text-red-500 flex-shrink-0">🚨 마감</span>
                  <span className="text-[13px] font-bold text-slate-700 truncate">
                    {new Date(todo.due_date).toLocaleString('ko-KR', {month:'short', day:'numeric', hour:'2-digit', minute:'2-digit'})}
                  </span>
                </div>
              )}
            </div>
